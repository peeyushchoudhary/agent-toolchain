...
def _open_commit_root(path: Path, *, create: bool) -> int:
    try:
    ...
    for directory in prune_dirs:
        ...
        for path in sorted(list(directory.glob("*.md")) + list(directory.glob("*.toml"))):
            if path.resolve() in expected:
                continue
            try:
                managed = GENERATED in path.read_text(encoding="utf-8", errors="replace")
            except OSError:
                continue
            if managed:
                operations.append({"action": "delete", "path": str(path.absolute())})
    return operations
