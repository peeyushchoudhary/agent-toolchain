...
## Definition review routing

A feature spec receives one fresh, read-only `reviewer` before Gate 1. At most one project domain
validator joins when the spec's own `## Horizontals` section says it moves a concern that
validator owns. `security-validator` joins when a declared safety surface moves. These seats have
distinct ownership, remain isolated, and produce one review round; there is no review panel or
finding quota. `product-steward` authors the spec and does not review its own artifact.

`reviewed_by:` records who actually read the definition. Rule F3 reports a moved concern whose
owning validator is absent, while the canonical methodology owns the blocker shape, correction,
scoped rereview, growth tripwire, and Gate 1 decision. Review output must land in an acceptance
criterion, a `Where it stops` line, an `Off-repo blockers` row, or a
`[NEEDS CLARIFICATION: <question>]` marker; free-standing review prose is not a fourth product
artifact.
