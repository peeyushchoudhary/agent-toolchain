...
flowchart TD
    ...
    S3 -. "fix cap, blocker, or any exit 2" .-> STOP["what stops the loop"]
    S5 -.-> STOP
```

The two back edges are the whole shape: a red gate and a blocking finding both return to **step 2**,
not to step 0, and the fix cap in §5 is what stops them cycling. Step 6 returns to **step 1** while
any task is uncommitted, so 7-9 run once per milestone and not once per task. The picture is
generated by nobody: `tests/test_execution_loop.py` requires every box to match a row of the table
below, and every script and option named in a box to be one that script's own parser accepts.
