...
| **How to start** | Install once, then explicitly invoke `methodology-management` in your project. See [Quickstart](#quickstart). |
...
Then open a project in Claude Code or Codex and explicitly invoke **`methodology-management`**
to assess or adopt it. **`project-onboarding`** remains a compatibility route to the setup
procedure. Requirements and installer behaviour: [install/README.md](install/README.md).
...
| Methodology management and compatibility routes | One explicit owner for assessment, setup, repair, product-document migration and upgrades. `project-onboarding` and `project-migration` remain explicit compatibility names; `project-conformance` remains an implicit read-only assessment route. | [Route mapping](docs/runbooks/methodology-management.md) · [Management](install/skills/methodology-management/SKILL.md) |
...
| Conformance after onboarding | Read-only aggregation of owning checks, including structured methodology readiness. A requested repair routes to management; assessment alone never starts setup, migration or adoption. | [`project-conformance`](install/skills/project-conformance/SKILL.md) |
| Migration of an onboarded repository | The explicit `project-migration` compatibility name routes to management's product-document procedure. It previews moves, preserves prose/history/links, and leaves substantive review metadata and methodology adoption to separate decisions. | [`project-migration`](install/skills/project-migration/SKILL.md) |
...
The execution methodology is at **v5.1**. Runtime readiness is now an owning structured status:
the approved identity, installed identity, project route, overlay and declared runtime dependencies
must agree before an adopted project reports `current`. Model assignments remain a separate
selective pilot policy; comparative quality, velocity and subscription efficiency are not yet
established.
...
| Methodology management | Explicit assessment/setup/repair/migration/upgrade coordination with compatibility routes for earlier skill names. |
...
`sync_methodology.py --repo <repo> --status-json` verifies the approved runtime inventory,
project route and overlay. A valid approved older bundle remains the execution source. A changed
global source or optional candidate is a finding to assess, not a fallback runtime or forced
upgrade.
...
adoption step. Model activation is a third separate rollout decision: this public tree keeps the
published candidate assignments unchanged and does not claim they are installed or validated on a
particular machine. Neither a passing unit suite nor a green push establishes release readiness.
...
| [onboarding-a-project.md](docs/runbooks/onboarding-a-project.md) | Scoped setup previews, authority boundaries and verification |
| [methodology-management.md](docs/runbooks/methodology-management.md) | Maintenance entry, compatibility routes, readiness and scope boundaries |
